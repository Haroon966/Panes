...
async function postHitl(path: string, approvalId: string): Promise<{ ok: boolean; message?: string; error?: string }> {
...
function ApprovalCard({ row }: { row: HitlApprovalRow }) {
  ...
  const onApprove = async () => {
    setBusy(true);
    const res = await postHitl('/api/agent/hitl/approve', row.approvalId);
    if (res.ok) {
  ...
  return (
    <div
      className={cn(
        'rounded-lg border border-terminalai-border bg-terminalai-overlay px-3 py-2.5 text-[12px] leading-snug',
...
export function HitlApprovalPanel() {
  ...
  return (
    <div className="shrink-0 space-y-2 border-t border-terminalai-borderSubtle bg-terminalai-surface px-3.5 py-2.5">
      ...
      {pending.map((row) => (
        <ApprovalCard key={row.approvalId} row={row} />
      ))}
      {done.length > 0 && (
