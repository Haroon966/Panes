...
export function MainLayout() {
  ...
  return (
    <div className="relative flex h-screen min-h-0 w-full min-w-0 flex-col bg-terminalai-base text-terminalai-text md:flex-row">
      {agentPanelOpen && (
        <div className="order-2 flex h-[min(42vh,340px)] min-h-0 w-full shrink-0 flex-col border-t border-terminalai-border md:order-1 md:h-auto md:w-[670px] md:border-r md:border-t-0">
          <ChatSidebar catalog={modelsCatalog} />
        </div>
      )}
      {agentPanelOpen && (
        <Tooltip>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className="fixed left-2 top-1/2 z-40 h-auto -translate-y-1/2 gap-1 rounded-lg border-terminalai-border bg-terminalai-elevated px-2 py-3 text-2xs text-terminalai-muted shadow-md hover:bg-terminalai-hover hover:text-terminalai-text md:left-0 md:rounded-l-none md:rounded-r-md md:border-l-0 md:pl-1"
            >
